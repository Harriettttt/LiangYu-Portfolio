...
class Trainer():
    ...
    def train(self):
        for episode in range(self.n_episode):
            ...
            if episode % 100 == 0:
                print(f'Episode: {episode}/{self.n_episode} | 平均奖励: {avg_reward:.2f} | 当前奖励: {episode_reward:.2f} | epsilon: {EPS_END + (EPS_START - EPS_END)* math.exp(-1. * self.agent.stepdone / EPS_DECAY):.3f}')
