...
max_seq_length = 256
full_dataset = NewsDataset('train.txt', label_map, max_seq_length=max_seq_length)
...
rf_model.fit(train_inputs, train_labels)
print("rf done")
...
train_accuracy, train_recall, train_precision, train_f1, train_report, train_cm = evaluate(rf_model, train_inputs, train_labels)
test_accuracy, test_recall, test_precision, test_f1, test_report, test_cm = evaluate(rf_model, test_inputs, test_labels)
...
def plot_and_save_confusion_matrix(cm, classes):
    ...
    plt.savefig('./rf_model/confusion_matrix.png')
    plt.close()
...
plot_and_save_accuracy(train_accuracy, test_accuracy, 'Accuracy', './rf_model/accuracy_plot.png')
